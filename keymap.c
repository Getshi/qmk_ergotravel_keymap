...
// NOTE to self: to compile, run qmk compile from anywhere within msys2 64bit, assuming keyboard and keymap default set

// TODO undo RALT on f layer, because that then only works to compose stuff that is on the f layer...
// put some stuff on left side for mouse on right.
// like make one of the extra thumb cluster keys enter, and one y. alternatively combo with unused alt to make space be enter and z be y
// simplest way is to make another layer "righthandonmouse"
